module REFRACT(
    ...
    // --- Shared Divider Logic (One module to minimize Area) ---
    ...
    always @(*) begin
        ...
        case (state)
            ...
            COMP_COEF: begin div_a = ($signed({1'b0, eta}) - $signed(sqrt_kgg_r)) << 12; div_b = (g2 == 0) ? 32'sd4096 : {{15{g2[16]}}, g2}; end
    ...
    // --- Corrected Fixed-point Shifting (Q4.12) ---
    wire signed [16:0] ax = (big_x - 17'sd32768) >>> 3; // (X-8)/8
    wire signed [16:0] ay = (big_y - 17'sd32768) >>> 3;
    
    // Manually cascading to control precision
    wire signed [16:0] ax2 = (ax * ax) >>> 12;
    wire signed [16:0] ax4 = (ax2 * ax2) >>> 12;
    wire signed [16:0] ax6 = (ax4 * ax2) >>> 12;
    wire signed [16:0] ax7 = (ax6 * ax) >>> 12;
    wire signed [16:0] ax8 = (ax4 * ax4) >>> 12;

    wire signed [16:0] ay2 = (ay * ay) >>> 12;
    wire signed [16:0] ay4 = (ay2 * ay2) >>> 12;
    wire signed [16:0] ay6 = (ay4 * ay2) >>> 12;
    wire signed [16:0] ay7 = (ay6 * ay) >>> 12;
    ...
    always @(posedge CLK or posedge RST) begin
        if (RST) begin
            ...
            state <= next_state;
            case (state)
                IDLE:      begin eta <= div_q[15:0]; eta2 <= (div_q[15:0] * div_q[15:0]) >> 12; iteration <= 9'd0; end
                GET_COOR:  begin 
                    big_x <= {1'b0, iteration[3:0], 12'd0}; // X in Q4.12
                    big_y <= {1'b0, iteration[7:4], 12'd0}; // Y in Q4.12
                    SRAM_WE <= 1'b0; 
                end
                COMP_G:    begin 
                    gx <= ax7 << 1; gy <= ay7 << 1; 
                    big_z <= 17'sd24576 - (ax8 << 1) - (ay8 << 1); // 6 - 2*ax8 - 2*ay8
                end
                COMP_KGG:  begin 
                    g2 <= ((gx * gx) >>> 12) + ((gy * gy) >>> 12) + 17'sd4096; 
                    kgg <= (((17'sd4096 - $signed({1'b0, eta2})) * g2) >>> 12) + $signed({1'b0, eta2}); // k*g2 formula
                end
                COMP_SQRT: begin sqrt_kgg_r <= sqrt_out[15:0]; end
                COMP_COEF: begin coef <= div_q[16:0]; end
                COMP_T:    begin t <= div_q[16:0]; end
                COMP_ZXY:  begin 
                    z_x <= big_x + (((t * coef) >>> 12) * gx >>> 12); // zx = X + t*coef*gx
                    z_y <= big_y + (((t * coef) >>> 12) * gy >>> 12); // zy = Y + t*coef*gy
                end
                WRITE_X:   begin SRAM_WE <= 1'b1; SRAM_A <= iteration << 1; SRAM_D <= z_x[15:0]; end
                WRITE_Y:   begin 
                    SRAM_WE <= 1'b1; SRAM_A <= (iteration << 1) + 9'd1; SRAM_D <= z_y[15:0]; 
                    iteration <= iteration + 9'd1; // Increment iteration
                end
